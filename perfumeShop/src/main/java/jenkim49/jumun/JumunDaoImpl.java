...
public class JumunDaoImpl implements JumunDao {
	...
	public Jumun jumun_insert_directly(Jumun jumun) throws Exception {
		...
		JumunDetail realJumunDetail = new JumunDetail();
		ArrayList<JumunDetail> realJumunDetailList = new ArrayList<JumunDetail>();
		...
		try {
			...
			pst = con.prepareStatement(JumunDetailSQL.JUMUNDETAIL_READ_JNO_CHECKOUT);
			pst.setInt(1, cnt);
			rs = pst.executeQuery();
			
			while(rs.next()) {
				realJumunDetailList.add(new JumunDetail(rs.getInt("jd_no"), rs.getInt("jd_item_qty"),
						rs.getInt("jd_items_price"), rs.getInt("j_no"), rs.getInt("p_no")));
			}
			...
			if(rs.next()) {
				realJumun = new Jumun(rs.getInt("j_no"), rs.getString("j_name"), rs.getInt("j_total_qty"), rs.getInt("j_total_price"), rs.getDate("j_date"), rs.getString("m_id"), realJumunDetailList);
			}
			pst.close();
			con.commit();
	...
	public Jumun jumun_readNo(int j_no) throws Exception {
		...
		try {
			...
			ArrayList<JumunDetail> realJumunDetailList = new ArrayList<JumunDetail>();
			pstmt = con.prepareStatement(JumunDetailSQL.JUMUNDETAIL_READ_JNO_CHECKOUT);
			pstmt.setInt(1, j_no);
			...
			if(rs.next()) {
				jumun = new Jumun(rs.getInt("j_no"), rs.getString("j_name"), rs.getInt("j_total_qty"), rs.getInt("j_total_price"), rs.getDate("j_date"), rs.getString("m_id"), realJumunDetailList);
			}
